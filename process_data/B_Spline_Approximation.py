...
class BS_curve(object):
    ...
    def get_knots(self,method ="average" ):
        ...
        self.m = self.n + self.p + 1
        # we only need m+1 knots
        # m+1个控制点，不算边缘（p+1)个0 ， （p+1)个1 ，有 （m+1)-2(p+1)个中间节点
        #中间节点个数
        knots_temp_size = self.m+1 -2*(self.p+1)
        #节点平均分布，步长 
        paras_temp = 1/(knots_temp_size +1)
        k_temp = 0
        for j in range(1,self.m+1 -2*(self.p+1)+1):
            k_temp  += paras_temp
            knots.append(k_temp)
...
class BS_curve2(object):

    def __init__(self,n,p,cp,knots):
        self.n = n # n+1 control points >>> p0,p1,,,pn
        self.p = p
